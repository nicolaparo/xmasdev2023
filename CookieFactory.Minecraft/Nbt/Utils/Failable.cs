...
namespace CookieFactory.Minecraft.Nbt.Utils
{
    ...
    public class Failable<T> : AbstractFailable, IFailable<T>
    {
        ...
        public IFailable<U> Then<U>(Func<T, U> further)
        {
            if (Failed)
                return Cast<U>();
            return new Failable<U>(() => further(Result), Description);
        }

        public IFailable<U> Cast<U>()
        {
            var result = Failed ? default : (U)(object)Result;
            return new Failable<U>(result, Exception, Description, Subfailures.Cast<IFailable<T>>().Select(x => x.Cast<U>()));
        }
        ...
        {
            return Flattened().Cast<IFailable<T>>();
        }
    }
}
