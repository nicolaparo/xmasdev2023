...
namespace CookieFactory.Minecraft.Nbt
{
    ...
    internal sealed class NbtBinaryWriter
    {
        ...
        {
            uint tmpValue = BitConverter.SingleToUInt32Bits(value);
            unchecked
            {
                if (swapNeeded)
                {
        ...
        {
            ulong tmpValue = BitConverter.DoubleToUInt64Bits(value);
            unchecked
            {
                if (swapNeeded)
                {
        ...
        public void Write(string value)
        {
            ...
            else
            {
                ...
                while (numLeft > 0)
                {
                    ...
                    byteLen = encoder.GetBytes(value.ToCharArray(), charStart, charCount, buffer, BufferSize, charCount == numLeft);
